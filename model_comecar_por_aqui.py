...
def heroi_pronto_por_nome(nomeHeroi):
    ...
    with engine.connect() as con:
        statement = text("""SELECT * FROM Heroi WHERE nome = :nome""")
        rs = con.execute(statement, nome=nomeHeroi)
        heroi = rs.fetchone()
        vida = heroi['fisico'] * 10
        if heroi == None:
            raise HeroiNaoExisteException

        # chamei a função pra verificar se possui item
        lista_item = itens_do_heroi.itens_em_uso_por_nome_do_heroi(nomeHeroi)

        if lista_item != []:
            for item in lista_item:
                # somando atributos
                dicionario = dict(heroi)
                dicionario['fisico'] += item['fisico']
                dicionario['magia'] += item['magia']
                dicionario['agilidade'] += item['agilidade']
                dicionario['vida'] = vida
            return dicionario

        # Se não tiver item, ele retorna o dicionário normal do status sem alterações
        dicionario = dict(heroi)
        dicionario['vida'] = vida
        return dicionario
...
def atacar_com_fisico(atacante, defensor):
    import math
    dano = atacante['fisico']
    nome_atac = atacante['nome']
    nome_def = defensor['nome']
    # prints maneiros
    # mensagem_de_ataque_fisico(dano,nome_atac,nome_def)
    # atualizar dados
    numero_ataques = atacante['agilidade'] / defensor['agilidade']
    chao = math.floor(numero_ataques)

    if chao > 1:
        dano_ataque = atacante['fisico'] * chao
        vida_atualizada = defensor['vida'] - dano_ataque
        defensor['vida'] = vida_atualizada

    elif chao <= 1:
        vida_atualizada = defensor['vida'] - atacante['fisico']
        defensor['vida'] = vida_atualizada
...
def atacar_com_magia(atacante, defensor):
    ...
    if chao > 1:
        dano_ataque = atacante['magia'] * chao
        vida_atualizada = defensor['vida'] - dano_ataque
        defensor['vida'] = vida_atualizada
        if defensor['vida'] <= 0:
            defensor['vida'] = 0
            return "Defensor morto"

    elif chao <= 1:
        dano_ataque = atacante['magia']
        vida_atualizada = defensor['vida'] - dano_ataque
        defensor['vida'] = vida_atualizada
        if defensor['vida'] <= 0:
            defensor['vida'] = 0
...
def lista_itens_em_uso_do_heroi(idHeroi):
    itens = itens_do_heroi.itens_do_heroi(idHeroi)
    lista_uso = []
    for i in range(len(itens)):
        if itens[i]['emUso'] == 1:
            lista_uso.append(itens[i])
    return lista_uso
...
'''
Ex13
Melhore sua função heroi_pronto_por_nome: agora, os o dicionario que representa o heroi é 
alterado pelos itens em uso. 
...
def criar_heroi(nome, agilidade, fisico, magia):
    with itens_do_heroi.engine.connect() as con:
        statement = "INSERT INTO Heroi (nome, agilidade, fisico, magia) VALUES (:nome,:agilidade,:fisico,:magia)"

        # upgrade da 21
        if magia + fisico + agilidade > 20:
            raise OverpowerException
        else:
            con.execute(statement, nome=nome, agilidade=agilidade,
                        fisico=fisico, magia=magia)
...
def dar_item_para_heroi(heroi, item):
    with itens_do_heroi.engine.connect() as con:
        id_heroi = heroi['id']
        id_item = item['id']
        statement = "INSERT INTO ItemDoHeroi (idItem,idHeroi) VALUES (:idItem,:idHeroi)"
        con.execute(statement, idHeroi=id_heroi, idItem=id_item)
...
def colocar_item_em_uso(heroi, item):
    with itens_do_heroi.engine.connect() as con:
        id_heroi = heroi['id']
        nome_item = item['nome']
        statement = "UPDATE Item SET emUso = 1 WHERE nome = :nome_item"

        # Upgrade da 25
        item_em_uso = itens_do_heroi.itens_do_heroi(id_heroi)
        if item_em_uso[0]['tipo'] == item['tipo'] and item_em_uso[0]['emUso'] == 1:
            raise HeroiJaUsaEsseTipoDeItemException
        else:
            con.execute(statement, nome_item=nome_item)
